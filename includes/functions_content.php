...
{
    global $heart, $db, $user, $lang, $G_PID, $G_PAGE, $settings;
    if ($element == "logged_info") {
        if ($user['uid']) {
            $class = "logged_info";
        ...
        if ($page['must_be_logged'] == 1 && !$user['uid']) {
        ...
        } else if ($page['must_be_logged'] == -1 && $user['uid']) {
            ...
            if ($G_PID == "register") {
                ...
                if (!$code) {
                    ...
                    // Brak kodu w bazie danych
                    if (empty($row)) {
                        $output = $lang['wrong_reset_key'];
                    } // Znalazło użytkownika z takim kodem
                    else {
                        $sign = md5($row['uid'] . $settings['random_key']);
                    }
                }
            } else if ($G_PID == "transfer_finalized") {
    ...
    } else if ($element == "services_buttons") {
        $services = "";
