...
SELECT `id` FROM `ss_pricelist` AS a
LEFT JOIN `ss_services` AS b ON a.service = b.id
WHERE b.id IS NULL;

DELETE FROM `ss_pricelist` WHERE `id` IN (
    SELECT `id` FROM `ss_tmp`
);
...
INSERT INTO `ss_tmp` (`id`)
SELECT `id` FROM `ss_payment_admin` AS pa
LEFT JOIN `ss_users` AS u ON pa.aid = u.uid
WHERE u.uid IS NULL;

DELETE FROM `ss_payment_admin` WHERE `id` IN (
    SELECT `id` FROM `ss_tmp`
);
